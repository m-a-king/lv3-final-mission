...
public class MatchMakingController {
    ...
    @GetMapping
    public ResponseEntity<List<Apply>> get(@RequestHeader("nickname") final String nickname,
                                           @RequestHeader("password") final String password) {
        final UserInfo userInfo = new UserInfo(nickname, password);
    ...
    public ResponseEntity<String> create(@RequestBody final UserInfo userInfo) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body("매치메이킹 신청이 완료되었습니다.");
    }
    ...
    public ResponseEntity<String> delete(@RequestBody final UserInfo userInfo) {
        ...
        return ResponseEntity.ok("매치메이킹 신청이 취소되었습니다.");
    }
}
